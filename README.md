...
1. Развертывание осуществляется с помощью workflow на основе файла `backend/.github/workflows/main.yml` по тригеру - push в ветку `master`.
...
DB_PORT 5432 - <порт соединения для PostgreSQL>
DOCKER_USERNAME - <имя пользователя для DockerHub>
DOCKER_PASSWORD - <пароль пользователя для DockerHub>
HOST 158.160.66.88 - <IP адрес удаленного сервера>
USER - <имя пользователя для соеднения с сервером>
...
4. Приложение будет доступно локально по адресу `http://choa.zapto.org/`
...
```
```
default - файл конфигурации внешего nginx на виртуальной машине в Яндекс облаке.
```
